...
header="Directory,Extension,Count,Size,Percent"
...
find "$dir" -type d | while read -r subdir; do
    if [ "$(ls -A "$subdir")" ]; then  # Check if directory has files
        ...
        find "$subdir" -type f | awk -F . '{if (NF>1) {print $NF} else {print "NoExtension"}}' | sort | uniq -c | awk '{printf "%s,%s,%s,%s\n", $2, $1, $1/total*100, ""}' total=$(find "$subdir" -type f | wc -l) | while read -r extension count percent _; do
